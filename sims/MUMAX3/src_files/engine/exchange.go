...
var (
	...
	D_D2d  = NewScalarParam("D_D2d", "J/m2", "D2d symmetry Dzyaloshinskii-Moriya strength", &dd2d2)
	lex2   exchParam // inter-cell Aex
	din2   exchParam // inter-cell Dind
	dbulk2 exchParam // inter-cell Dbulk
	dd2d2  exchParam // inter-cell D_D2d
...
func AddExchangeField(dst *data.Slice) {
	...
	case !bulk && !inter && D2d:
		cuda.AddDMID2d(dst, M.Buffer(), lex2.Gpu(), dd2d2.Gpu(), ms, regions.Gpu(), M.Mesh()) // dmi+exchange
	case inter && bulk && D2d:
		util.Fatal("Cannot have multiple DMI at the same time")
	case inter && bulk && !D2d:
		util.Fatal("Cannot have multiple DMI at the same time")
	case inter && !bulk && D2d:
		util.Fatal("Cannot have multiple DMI at the same time")
